...
class Solution:
    def solveSudoku(self, board):
        ...
        def can_place(row, col, val):
            return (
                not Solution.rows[row][val - 1]
                and not Solution.cols[col][val - 1]
                and not Solution.boxes[row // 3 * 3 + col // 3][val - 1]
            )
        ...
        def search(x, y, word_idx, visited) -> bool:
            if (
                ...
                if word_idx == len(word) - 1:
                    ...
                    visited.add((x, y))
                    return (
                        search(x, y - 1, word_idx + 1, visited.copy())
                        or search(x, y + 1, word_idx + 1, visited.copy())
                        or search(x - 1, y, word_idx + 1, visited.copy())
                        or search(x + 1, y, word_idx + 1, visited.copy())
                    )
